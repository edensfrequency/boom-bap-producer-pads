...
- Presets â€” save/load/delete named presets independent of any DAW project
  (ADR-0009)
- Real file-system sample browser with search, replacing the placeholder
  (ADR-0010, search added later â€” ADR-0016)
...
- Standalone build (`Boom Bap Producer Pads.exe`) â€” runs without a DAW,
...
- Sample-accurate step triggering â€” steps now start on the exact sample,
...
- Turntable scratch sounding like "a high-pitched jumbled" version of the
  sample rather than a real scratch â€” root-caused to an unclamped position
...
- Step sequencer never advancing in the standalone build â€” confirmed via
...
### Added
- 16-pad one-shot sample engine â€” load a sample per pad via file chooser,
  right-click menu, or drag-and-drop; trigger via MIDI (notes 36â€“51, real
...
### Known limitations
- No sample trimming/start-point â€” full sample plays from 0 every time
...
- Windows only â€” no macOS/Logic build yet
