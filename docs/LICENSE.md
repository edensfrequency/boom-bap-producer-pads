![banner-image.png](../assets/banner-image.png)

<hr>

# License (End-User Terms)

> **Draft â€” private, unreleased build.** Boom Bap Producer Pads hasn't had
> a public release yet. These terms describe how the plugin is intended
> to be licensed to end users once it does; they aren't final until a
> release actually ships.

## What you get

A license to install and use compiled builds of Boom Bap Producer Pads
(the VST3 plugin and/or the standalone app) for your own music production
â€” personal or commercial. You do not need to credit "Boom Bap Producer
Pads" in tracks you make with it.

## What you don't get

- The right to redistribute, sell, sublicense, or rehost the plugin
  binary yourself â€” get it from the official source.
- The right to reverse-engineer, decompile, or extract the source code
  from a compiled build, beyond what applicable law permits regardless of
  this license.
- Ownership of the "Boom Bap Producer Pads" name, its UI design, or its
  bundled assets â€” those remain the creator's ("AdamNubian", distributed
  by "EdensFrequency").
- Any warranty. The plugin is provided **as is**, without warranty of any
  kind, express or implied.

## Third-party components

This plugin is built on the [JUCE](https://juce.com) framework, which
carries its own license terms separate from this one: JUCE is
dual-licensed under the
[AGPLv3](https://www.gnu.org/licenses/agpl-3.0.en.html) and the
commercial [JUCE End User Licence Agreement](https://juce.com/legal/juce-9-licence/).
This affects what you may do with JUCE itself, not with compiled builds
of this plugin, which you're free to use under the terms above.

## Questions

For anything not covered here â€” licensing for redistribution, commercial
bundling, or anything else outside normal end-user use â€” reach out to the
creator directly rather than assuming; nothing here is a substitute for
that conversation.
